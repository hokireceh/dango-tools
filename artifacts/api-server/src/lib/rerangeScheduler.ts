...
async function checkBudgetStop(bots: typeof gridBotsTable.$inferSelect[]): Promise<void> {
  for (const bot of bots) {
    if (!bot.isActive || bot.budgetStop === null) continue;
    const pnl = Number(bot.totalPnl);
    const limit = Number(bot.budgetStop);
    if (pnl <= -limit) {
      await db
      ...
      await db.insert(botLogsTable).values({
        botId: bot.id,
        eventType: "TOGGLE",
        message: `Bot otomatis dinonaktifkan — Budget Stop tercapai. PnL $${pnl.toFixed(2)} melewati batas -$${limit.toFixed(2)}.`,
        priceAtEvent: null,
      });
      logger.warn(
        { botId: bot.id, botName: bot.name, pnl, budgetStop: limit },
        "Bot dinonaktifkan oleh Budget Stop"
      );
    }
  }
}
