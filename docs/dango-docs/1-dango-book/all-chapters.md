> SOURCE: https://docs.dango.exchange/print.html
...
- Converts the token amount to USD: depositValue=amount×1.

- Increment userState.margin by depositValue.
...
- Computes availableMargin (see §8), clamped to zero.

- Ensures the requested amount does not exceed availableMargin.

- Deducts the amount from userState.margin.

- Converts USD to settlement currency tokens at the fixed $1 rate (floor-rounded to base units).
...
equity=collateralValue+∑unrealisedPnl−∑accruedFunding


where collateralValue is the USD value of the user’s deposited margin (`userState.margin`).


Per-position unrealised PnL is:


unrealisedPnl=size×(oraclePrice−entryPrice)


and accrued funding is:


accruedFunding=size×(fundingPerUnit−entryFundingPerUnit)
...
IM=∑∣size∣×oraclePrice×imr


where imr is the per-pair initial margin ratio. IM is the minimum equity required to open or hold positions. It is used in two places:
...
MM=∑∣size∣×oraclePrice×mmr


where mmr is the per-pair maintenance margin ratio (always ≤imr). A user becomes eligible for liquidation when:
...
reservedPerOrder=∣openingSize∣×limitPrice×imr
...
available=max(0,equity−usedMargin−reservedMargin)
...
| Buy (+) | Short (−) | min(size,−position) | size−closing 

| Sell (−) | Long (+) | max(size,−position) | size−closing 

| Same direction | Any | 0 | size 
...
targetPrice=oraclePrice×(1+maxSlippage)
...
Limit orders: targetPrice=limitPrice (oracle price is ignored).


A price constraint is violated when:


- Bid: execPrice>targetPrice

- Ask: execPrice<targetPrice
...
- remainingSize=0 — the taker is fully filled.
...
takerFillSize={min(remainingSize,makerSize)max(remainingSize,makerSize)​if bid (both positive)if ask (both negative)​


makerFillSize=−takerFillSize
...
equity≥projectedIM+projectedFee+reservedMargin


where projectedIM is the initial margin assuming the full order fills (see Margin §5) and projectedFee is


∣size∣×oraclePrice×takerFeeRate
...
- The taker’s open_order_count and reserved_margin are decremented.

- The taker continues matching deeper in the book — no fill occurs for the self-matched order.
...
pnl=∣closingSize∣×(fillPrice−entryPrice)


Short closing (buying to close):


pnl=∣closingSize∣×(entryPrice−fillPrice)
...
entryPrice←∣newSize∣∣oldSize∣×oldEntry+∣openingSize∣×fillPrice​
...
- Closing a long: longOI−=∣closingSize∣

- Closing a short: shortOI−=∣closingSize∣

- Opening a long: longOI+=∣openingSize∣

- Opening a short: shortOI+=∣openingSize∣
...
fee=∣fillSize∣×fillPrice×feeRate
...
userState.margin−=fee


state.vaultMargin+=fee


Fees from the vault to itself are skipped (no-op). Processing fees first ensures collected fees augment vaultMargin before any vault losses are absorbed.
...
A negative vaultMargin represents a deficit (bad debt not yet recovered via ADL).
...
- Price is aligned to the pair’s tick size (limitPricemodtickSize=0)
...
marginToReserve=∣openingSize∣×limitPrice×imr


availableMargin≥marginToReserve


If the check passes, `reserved_margin` is increased by marginToReserve and `open_order_count` is incremented. This is the 0 %-fill scenario check — it ensures the user can afford the order even if nothing fills immediately.
...
- Buy: limitPrice≥bestAsk

- Sell: limitPrice≤bestBid
...
Each pair has a maxAbsOI parameter enforcing a per-side cap:


- Long opening: longOI+openingSize≤maxAbsOI

- Short opening: shortOI+∣openingSize∣≤maxAbsOI

The constraint is checked before matching and does not apply to reduce-only orders (which have zero opening size). Long and short OI limits are independent but share the same maxAbsOI parameter.
...
- reserved_margin is released (subtracted from the user’s total).

- open_order_count is decremented.

- If the user state is now empty (no positions, no open orders, no pending unlocks), it is deleted from storage.
...
Impact bid — the volume-weighted average price (VWAP) obtained by selling impactSize worth of base asset into the bid side.

- Impact ask — the VWAP obtained by buying impactSize worth from the ask side.

The premium is then:


premium=oraclemax(0,impactBid−oracle)−max(0,oracle−impactAsk)​


If either side has insufficient depth to fill impactSize, its max(0,…) term contributes zero. When both sides lack depth, the premium is zero.
...
When fundingPeriod has elapsed since the last collection, the same cron invocation finalises the funding rate:


- Average premium:
avgPremium=premiumSum/premiumSamples

- Clamp to the configured bounds:
rate=clamp(avgPremium,[−maxAbsFundingRate,+maxAbsFundingRate])

- Funding delta — scale by the actual elapsed interval and oracle price:
fundingDelta=rate×interval×oraclePrice

- Accumulate into the pair-level running total:
fundingPerUnit+=fundingDelta

- Reset accumulators: premiumSum←0, premiumSamples←0, lastFundingTime←now.
...
entryFundingPerUnit←fundingPerUnit
...
equity=collateralValue+∑unrealisedPnl−∑accruedFunding
MM=∑∣positionSize∣×oraclePrice×mmr
where mmr is the per-pair maintenance-margin ratio. An account becomes liquidatable when
equity<MM
...
mmContribution=∣size∣×oraclePrice×mmr
...
- closeSize=min(⌈oraclePrice×mmrdeficit​⌉,∣size∣)

- deficit−=closeSize×oraclePrice×mmr


This produces a vector of (pairId,closeSize) entries. Each closeSize has the opposite sign of the existing position (a long is closed with a sell, a short with a buy). Only positions that contribute to the deficit are touched and they may be partially closed when the deficit is small relative to the position.
...
Counter-party selection: Positions are indexed by the tuple (pairId,entryPrice,user). For a long being liquidated (selling), the system finds shorts with the highest entry price (most profitable) first. For a short being liquidated (buying), it finds longs with the lowest entry price first.1


Bankruptcy price: The fill price at which the liquidated user’s total equity would be exactly zero:


bp=oraclePrice−closeAmountequity​(for longs)


bp=oraclePrice+closeAmountequity​(for shorts)
...
rawFee=closedNotional×liquidationFeeRate


remainingMargin=max(0,margin+userPnlAfterCloses)


fee=min(rawFee,remainingMargin)
...
badDebt=∣min(0,margin after settlement)∣
...
AlicePnL=1×($47,500−$50,000)=−$2,500


Liquidation fee


closedNotional=1×$47,500=$47,500


rawFee=$47,500×0.1%=$47.50


remainingMargin=max(0,$3,000−$2,500)=$500
...
margin−=$47.50(fee)⇒$2,952.50


margin+=(−$2,500)(PnL)⇒$452.50


Final margin is positive — no bad debt.


insuranceFund+=$47.50(fee revenue)
...
bp=$46,000−1−$1,000​=$46,000+$1,000=$47,000
...
CharliePnL=1×($47,000−$50,000)=−$3,000


margin:$3,000+(−$3,000)=$0(zeroed by construction)


Dana’s PnL at bankruptcy price:


DanaPnL=−1×($47,000−$55,000)=$8,000


DanaMargin:$10,000+$8,000=$18,000


Liquidation fee


remainingMargin=max(0,$3,000−$3,000)=$0


fee=$0(no margin left)
...
CharliePnL=1×($46,000−$50,000)=−$4,000


Liquidation fee


remainingMargin=max(0,$3,000−$4,000)=$0
...
badDebt=$1,000,margin←$0


insuranceFund:$500−$1,000=−$500
...
The LP specifies a USD margin amount depositMargin to transfer from their trading margin to the vault.
effectiveSupply=vaultShareSupply+virtualShares
effectiveEquity=vaultEquity+virtualAssets
sharesToMint=⌊effectiveSupply×effectiveEquitydepositMargin​⌋
...
releaseValue=effectiveEquity×effectiveSupplysharesToBurn​
The fund is not released immediately. A cooldown is initiated, with the ending time computed as:
endTime=currentTime+vaultCooldownPeriod
Once endTime is reached, the contract credits the released USD value back to the LP’s trading margin.
...
vaultEquity=vaultMarginValue+∑unrealisedPnl−∑accruedFunding
where vaultMargin is the vault’s internal USD margin (updated in-place during settlement), and the sums run over all of the vault’s open positions.
If effectiveEquity is non-positive the vault is in catastrophic loss and both deposits and withdrawals are disabled.
...
pairMargin=vaultAvailableMargin×vaultTotalWeightvaultLiquidityWeight​
where vaultAvailableMargin=max(0,equity−usedMargin) and usedMargin is the sum of initial margin across all vault positions (see Margin §8).


### Quote size

Each side receives half the allocated margin, capped by a per-pair maximum:
halfMargin=2pairMargin​
quoteSize=min(oraclePrice×imrhalfMargin​,maxQuoteSize)
where imr is the initial margin ratio.


### Bid price

rawBid=oraclePrice×(1−halfSpread)
Snap down to the nearest tick:
bidPrice=rawBid−(rawBidmodtickSize)
Book-crossing prevention: if bidPrice≥bestAsk, clamp to bestAsk−tickSize.
Skip if bidPrice≤0 or notional is below the minimum order size.


### Ask price

rawAsk=oraclePrice×(1+halfSpread)
Snap up to the nearest tick (ceiling):
askPrice={rawAskrawAsk−(rawAskmodtickSize)+tickSize​if rawAskmodtickSize=0otherwise​
Book-crossing prevention: if askPrice≤bestBid, clamp to bestBid+tickSize.
...
Commissions are taken from the trading fee after the protocol treasury has claimed its share. The system can be disabled globally by setting active=false in the referral parameters, which causes the commission pass to be skipped entirely.
...
- Commission rate (cr) — the fraction of the post-protocol-cut fee that the referral system distributes at a given level. This rate is tiered: it increases as the referrer’s direct referees accumulate more 30-day rolling trading volume (see §6b). The chain owner can also set a per-user override (see §6a).

- Fee share ratio (sr) — the fraction of the level-1 commission that the direct referrer gives back to the referee as a rebate. For example, if the commission rate is 20 % and the share ratio is 50 %, the referee receives 10 % and the referrer keeps 10 %. The share ratio is capped at 50 % and can only increase once set.
...
A user opts in as a referrer by calling `SetFeeShareRatio` with a shareRatio value. The share ratio determines what fraction of the level-1 commission the referrer gives back to the referee (see §5a).


Eligibility: the user must have accumulated enough lifetime trading volume:


lifetimeVolume≥volumeToBeReferrer
...
- shareRatio≤50% — the maximum share ratio a referrer can set.
...
- During account creation — the RegisterUser message on the account factory accepts an optional referrer field. If provided, the factory forwards a SetReferral message to the perps contract.

- After account creation — the referee (or an account they own) calls SetReferral directly on the perps contract.

Constraints:


- A user cannot refer themselves (referrer=referee).
...
When a referral is registered, a per-referee statistics record is initialised for the (referrer, referee) pair, and the referrer’s refereeCount is incremented in today’s cumulative data bucket.
...
protocolFee=fee×protocolFeeRate


vaultFee=fee−protocolFee


The protocol fee is routed to the treasury and is not affected by referrals. Referral commissions are computed against vaultFee — i.e. the remainder of the fee after the protocol has taken its cut.
...
Let cr1​ be the commission rate of the direct referrer (see §6) and sr be that referrer’s fee share ratio.


The referee (fee payer) receives:


refereeShare=vaultFee×cr1​×sr


The direct referrer receives:


referrerCommission=vaultFee×cr1​×(1−sr)


Equivalently, the total level-1 commission is vaultFee×cr1​, split between referee and referrer by the share ratio.


### 5b. Levels 2–5 — upstream referrers

The algorithm walks up the referral chain from the direct referrer. At each level n (2≤n≤5), let crn​ be the commission rate of the n-th referrer and maxCr be the maximum commission rate seen at any prior level (initialised to cr1​).


marginaln​=max(0,crn​−maxCr)


commissionn​=vaultFee×marginaln​


After computing commissionn​, update:


maxCr←max(maxCr,crn​)


If crn​≤maxCr, the referrer at level n receives nothing. The chain walk stops early if a referrer at level n has no referrer of their own, or after level 5.
...
| User | Commission rate (cr) | Fee share ratio (sr) | Referrer 
...
Trade. User D trades $10 m taker volume, pays $1,000 in fees. Assume protocolFeeRate=0 so vaultFee=$1,000.


| Level | User | cr | maxCr | Receives 


| 1 (referee D) | D | — | — | $1,000×cr×sr=$60 

| 1 (referrer C) | C | 15 % | — | $1,000×cr×(1−sr)=$90 

| 2 | B | 20 % | 15 % | $1,000×(cr−maxCr)=$50 

| 3 | A | 30 % | 20 % | $1,000×(cr−maxCr)=$100 
...
Counter-example. Now User F signs up under User E (40 % commission rate) and trades. Since E’s 40 % exceeds every upstream referrer, no upstream commissions are paid — only E and F split the level-1 commission of vaultFee×40%.


## 6. Commission rate

The commission rate cr for a referrer determines the fraction of the post-protocol-cut fee that the referral system distributes at that level.


### 6a. Override

The chain owner can set (or remove) a per-user override via `SetCommissionRateOverride`. When present, this value is used directly, bypassing the volume-tiered lookup. Users with an override also bypass the volumeToBeReferrer requirement when calling `SetFeeShareRatio`.


### 6b. Volume-tiered lookup

When no override exists, cr is derived from the referrer’s direct referees’ 30-day rolling trading volume:


- Load the referrer’s latest cumulative referral data; let Vnow​ be its refereesVolume field.

- Load the cumulative data at tnow​−30days; let Vstart​ be its refereesVolume field.

- Compute the windowed volume:
windowRefereesVolume=Vnow​−Vstart​

- Walk the commissionRatesByVolume map and select the entry with the highest volume threshold ≤windowRefereesVolume.

- If no tier qualifies, use commissionRateDefault.
...
- The referee’s bucket: volume and commissionSharedByReferrer increment.

- The direct referrer’s bucket: refereesVolume and commissionEarnedFromReferees increment.

- Upstream referrers: only commissionEarnedFromReferees increments (and only if they received a non-zero commission).
...
On the first trade of each day by a given direct referee, the referrer’s cumulativeActiveReferees field in today’s cumulative bucket is incremented. Subsequent trades by the same referee on the same day do not increment it again. This is tracked via the lastDayActive field on `RefereeStats`: if lastDayActive=today, it is a new active day.
...
The initial margin ratio (IMR) sets maximum leverage (1/imr). The maintenance margin ratio (MMR) sets the liquidation threshold. Both are per-pair.
...
- Compute the 99.5th-percentile absolute daily return r99.5​.

- Apply a liquidation-delay factor k (typically 2–3) to account for the time between the price move and the liquidation execution:
mmr=r99.5​×k
...
liquidationFeeRate≤mmr−takerFeeRate
...
maxAbsFundingRatedaily​≤Tliq​imr​


where Tliq​ is the number of days it should take sustained max-rate funding to liquidate a fully leveraged position. For Tliq​=3 days and imr=5%:


maxRate=30.05​≈1.67% per day
...
maxAbsOI≤mmr×tailLossFactorvaultEquity×wpair​​


where wpair​ is the pair’s weight fraction and tailLossFactor (2–5) is a safety multiplier reflecting how many times maintenance margin the vault could lose in a tail event.
...
tickSize≈oraclePrice×10−4 to 10−3
...
halfSpread≥σintraday​+makerFeeRate


where σintraday​ is the standard deviation of intra-block price changes. A larger spread protects against adverse selection but reduces fill probability.
...
maxQuoteSize≤maxAbsOI×f,f∈[0.3,0.8]
...
- Compute volatility — For each asset, compute r99.5​ (daily 99.5th percentile absolute return) and σintraday​ (hourly return standard deviation).

- Set margin ratios — Derive MMR from r99.5​ (§1.1), then IMR as a multiple of MMR. Cross-check against peer benchmarks (§1.2).

- Set fees — Choose maker/taker/liquidation fee rates satisfying §2.1 and the invariant in §1.3.

- Set funding — Pick funding_period, derive max_abs_funding_rate (§3.1), and calibrate impact_size (§3.2).

- Size exposure — Set max_abs_oi from vault equity and tail-risk tolerance (§4.1).

- Set order constraints — Choose min_order_size and tick_size (§4.2, §4.3).

- Configure vault — Set vault_half_spread, vault_max_quote_size, and vault_liquidity_weight per pair (§5), and vault_cooldown_period globally.
...
- Simulate — send an UnsignedTx to estimate gas (see §2.8).

- Set gas limit — use the simulation result, adding ~770,000 for signature verification overhead.

- Build SignDoc — assemble {sender, gas_limit, messages, data}.

- Sign — sign the SignDoc with the chosen method.

- Broadcast — submit the signed Tx via broadcastTxSync (see §2.9).
...
pm​ is the price, denoted as the units of quote asset per one unit of base asset (that is, y over x), of trading an infinitesimal amount of one asset to the other. On a graph, it is the slope of the tangent line that touches the curve at the point (x,y).


Let’s imagine the pool starts from the state of (x0​=5,y0​=2); marginal price pm​(x0​,y0​)=0.4 USD per BTC.


At this time, if a trader swaps 2 units of USD to 2.5 units of BTC, the state would move to the point (x1​=2.5,y1​=4), marginal price pm​(x1​,y1​)=1.6 USD per BTC.
...
- The pool is parameterized by spread ps​ and “bin size” Δp.

- Start from the marginal price pm​(x0​,y0​) (we denote this simply as pm​ from here on).

The pool would not place any order here. We say the total order size here is zero.


- Move on to the “bin” at price p0​=pm​+21​ps​ (marginal price plus the half spread).

This is the price at which the pool will place its first SELL order.

- Using the approach discussed above, find the total order size between the prices p0​ and pm​. This is the size of the order to be place here.


- Move on to the next “bin”, at price p1​=p0​+Δp.

Using the approach discussed above, find the total order size between the prices p1​ and p0​.

- Subtract the total order size between pm​ and p0​, this is the order size to be placed here.


- Do the same for p2​, p3​, … until liquidity runs out (total order size ≥x0​).

With the same approach, we can work out all the BUY orders for prices below pm​.


For the xyk curve, the orders are visualized as follows (based on the state x0​=1, y0​=200 and parameters ps​=0.2, Δp=0.1):
...
As example for a concentrated liquidity curve, the Solidly curve I(x,y)=x3y+xy3 results in the following orders:
...
- Pyth prices come with a confidence range, meaning a range it thinks there’s a 95% probability the true price is in. Our parameter ps​ should be configured to be similar or larger than this.
...
Q4 2024Informal SystemsFormal specification of JMT in Quintblog • spec
...
- The chain owner sends a transaction containing the a message in the following schema:
{
"upgrade": {
"height": 12345,
"cargo_version": "1.2.3",
...
- The chain finalizes the block right before the upgrade height (12344 in this example). At the upgrade height (12345), during FinalizeBlock, Grug app notices the upgrade height is reached, but the chain isn’t using the correct version (1.2.3), so it performs a graceful halt of the chain by returning an error in ABCI FinalizeBlockResponse. The upgrade height (12345) is not finalized, with no state change committed.

- The node operator replaces the node software on the server with the correct version (1.2.3), and restart the service.

- CometBFT attempts FinalizeBlock of the upgrade height (12345) again. Grug app notices the upgrade height is reached, and the software is of the correct version. It runs the upgrade logic specified in App::upgrade_handler (if any), and then resumes processing blocks.
...
- Perform another benchmark to measure how much time it takes for the host to execute each host function (e.g. addr_validate or secp256k1_verify). Based on this, assign a proper gas cost for each host function.
...
We have established that 1 second corresponds to 10^10 gas units. Therefore, `secp256k1_verify` costing 0.188 millisecond means it should cost 1010×0.077×10−3 = 770,000 gas.
...
If user liquidity is prefectly balanced, that is, Aa​=Bb​, fee rate should be zero: f(A,B,a,b)=0.

- If user liquidity is prefertly unbalanced, that is, one-sided (e.g. a=0 but b=0), then the fee rate should be a value such that if the attack is carried out, the output is equal to doing a swap normally.
...
Consider the case where the user liquidity is unbalanced. Without losing generality, let’s suppose Aa​>Bb​. That is, the user provides a more than abundant amount of A, and a less than sufficient amount of B.
...
Here, L+ll​ is the portion of the pool’s liquidity owned by the user. We can work out its expression as:


L+ll​=II′​−fII′​−1​=r−fr−1​


where r=II′​, which represents how much the invariant increases as a result of the added liquidity.


### Scenario 2

In the second scenario, the user does a swap of Δa amount of A into (1−s)Δb amount of B, where s is the swap fee rate, which is a constant. The swap must satisfy the invariant:


I(A,B)=I(A+Δa,B−Δb)


The user now has A−Δa amount of A and B+(1−s)Δb amount of B.
...
f=r−(r−1)A−ΔaA+a​=r−(r−1)B+(1−s)ΔbB+b​


We can use the first equation to work out either Δa or Δb, and put it into the second equation to get f.
...
- a margin account has collateral assets A1​,A2​,…,An​ and debts B1​,B2​,…,Bm​

- the price of an asset X is PX​

- the collateral power of an asset X is CX​

The account’s utilization is:


U=∑i=1n​CAi​​PAi​​Ai​∑j=1m​PBj​​Bj​​


In the `backrun` function, the margin account asserts U≤1. If not true, it throws an error to revert the transaction.
...
- Rust does not provide fixed-point decimal types, which are commonly used in financial applications (we don’t want to deal with precision issues with floating-point numbers such as 0.1 + 0.2 = 0.30000000000000004). Additionally, there are concerns over floating-point non-determinism, hence it’s often disabled in blockchains.

- Grug uses JSON encoding for data that go in or out of contracts. However, JSON specification (RFC 7159) only guarantees support for integer numbers up to (2**53)-1. Any number type that may go beyond this limit needs to be serialized to JSON strings instead.
...
- Call the taxman’s withhold_fee method. This ensures the tx’s sender has enough fund to afford the tx fee.

- Call the sender’s authenticate method in normal (i.e. non-simulation) mode. Here the sender performs authentication (which is skipped in simulation mode).
...
- Call taxman’s withhold_fee method.
...
- Call sender’s authenticate method.
...
- This said, a SubMessage can fail without aborting the tx, if it’s configured as such (with SubMessage::reply_on set to ReplyOn::Always or ReplyOn::Error). ↩
...
- Initialize the ~/.dango directory:
dango init
